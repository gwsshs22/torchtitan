...
"""
Parallelization for Hunyuan-MoE model.
Adapted from solar_open/infra/parallelize.py with modifications:
- QK norm TP (SequenceParallel on head dimension)
- Weight tying handled after parallelisms
"""
...
def apply_non_moe_tp(
    ...
    NOTE: Weight tying (tok_embeddings <-> output) is NOT done here because
    RowwiseParallel (Shard(1)) for embeddings and ColwiseParallel (Shard(0))
    for output are incompatible shardings for a shared weight tensor.
    Weight tying is handled at checkpoint save/load time via state_dict_adapter.
    """
    parallelize_module(
        model,
        tp_mesh,
    ...
    for transformer_block in model.layers.values():
        layer_plan = {
            ...
            # QK norm: operates on head_dim within each head, sharded across heads
